...
SELECT
    dt,
    -- Fix Gusta #2: provider vazio ('') tratado como 'Nao identificado'
    COALESCE(NULLIF(TRIM(sub_vendor_id), ''), NULLIF(TRIM(vendor_id), ''), 'Nao identificado') AS provider,
    ...
    SUM(turnover_total) AS turnover_total,
    SUM(ggr_total) AS ggr_total,
    -- Hold Rate por provedor
    ...
    SUM(jackpot_contribution) AS jackpot_contribution,
    -- Free Spins
    SUM(free_spins_bet) AS free_spins_bet,
    SUM(free_spins_win) AS free_spins_win
FROM multibet.fact_casino_rounds
GROUP BY dt, COALESCE(NULLIF(TRIM(sub_vendor_id), ''), NULLIF(TRIM(vendor_id), ''), 'Nao identificado');
"""
...
SELECT
    dt,
    CASE
        -- Prioridade 1: categoria canonica do bireports (Slots, Live)
        WHEN game_category IN ('Slots', 'Live') THEN game_category
        -- Prioridade 2: crash/instant games identificados por nome
        WHEN game_name ILIKE ANY (ARRAY[
            '%Aviator%', '%Spaceman%', '%High Flyer%',
            '%Mines%', '%Plinko%', '%Go Rush%',
            '%Big Bass Crash%', '%Penalty%', '%Crash%',
            '%JetX%', '%Rocket%', '%Balloon%',
            '%Dice%', '%Limbo%', '%Wheel%'
        ]) THEN 'Crash/Instant'
        -- Fallback: categoria original ou 'Outros'
        ELSE COALESCE(NULLIF(game_category, ''), 'Outros')
    END AS category,
    COUNT(DISTINCT game_id) AS qty_games,
    SUM(qty_players) AS qty_players,
    SUM(total_rounds) AS total_rounds,
...
SELECT
    dt,
    game_id,
    -- Fix Gusta #1: jogos 'Desconhecido' mostram game_id entre parenteses
    CASE WHEN game_name = 'Desconhecido'
         THEN 'Desconhecido (' || game_id || ')'
         ELSE game_name END AS game_name,
    -- Fix Gusta #2: provider vazio tratado como 'Nao identificado'
    COALESCE(NULLIF(TRIM(sub_vendor_id), ''), NULLIF(TRIM(vendor_id), ''), 'Nao identificado') AS provider,
    -- Fix Gusta v4.1: categoria canonica com override pra crash/instant games
    CASE
        WHEN game_category IN ('Slots', 'Live') THEN game_category
        WHEN game_name ILIKE ANY (ARRAY[
            '%Aviator%', '%Spaceman%', '%High Flyer%',
