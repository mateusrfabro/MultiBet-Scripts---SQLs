...
# 1. Fix View - JOIN spend por source (via dim_marketing_mapping como ponte)
...
SELECT
    ...
    -- Spend mensal por SOURCE (somado da fact_attribution via source)
    s.monthly_spend,
    -- ROI D30
...
LEFT JOIN (
    -- Spend agrupado por mes + source (usando dim_marketing_mapping como ponte)
    SELECT
...
""")
print("View vw_cohort_roi corrigida!")

# 2. Validar - Google Ads ROI por Safra
print()
...
""", fetch=True)
print(f"{'Safra':<8} {'Players':>8} {'LTV D30':>9} {'GGR D30':>13} {'Spend':>14} {'ROI%':>8} {'2nd%':>6}")
print("="*70)
for r in rows:
    sp = f"R${float(r[4]):>11,.0f}" if r[4] else "        N/A"
    roi = f"{float(r[5]):.1f}%" if r[5] else "   N/A"
    print(f"{r[0]:<8} {r[1]:>8,} R${float(r[2]):>7,.0f} R${float(r[3]):>11,.0f} {sp} {roi:>8} {float(r[6]):.1f}%")

# 3. Todas as sources
print()
print("=== TODAS AS SOURCES - ROI D30 (ultimos 3 meses) ===")
rows2 = execute_supernova("""
    SELECT month_of_ftd, source, qty_players, avg_ltv_d30, total_ggr_d30,
           monthly_spend, roi_d30_pct, pct_2nd_deposit
    FROM multibet.vw_cohort_roi
    WHERE month_of_ftd >= '2026-01'
      AND source IN ('google_ads','meta_ads','multi_channel','organic','unmapped_orphans')
    ORDER BY month_of_ftd, source
""", fetch=True)
print(f"{'Safra':<8} {'Source':<22} {'Players':>8} {'LTV D30':>9} {'GGR D30':>13} {'Spend':>14} {'ROI%':>8}")
print("="*85)
for r in rows2:
    sp = f"R${float(r[5]):>11,.0f}" if r[5] else "        N/A"
    roi = f"{float(r[6]):.1f}%" if r[6] else "   N/A"
    print(f"{r[0]:<8} {r[1]:<22} {r[2]:>8,} R${float(r[3]):>7,.0f} R${float(r[4]):>11,.0f} {sp} {roi:>8}")
